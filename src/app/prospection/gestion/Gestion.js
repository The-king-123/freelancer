...
function Gestion() {
        ...
        if (data.length > 0) {
            glicthKeyList = data.map((code, key) => (
                <div key={key} style={{ padding: 4 }}>
                    <div className={(themeLight ? 'w3-light-grey' : 'w3-black') + " w3-round w3-padding w3-flex-row w3-flex-center-v"}>
    ...
    const reloadProducts = (data) => {
        ...
        if (data.length > 0) {
            glitchProduct = data.map((product, key) => (
                <div key={key} style={{ padding: 4 }}>
                    <div onClick={() => showThisProduct(product)} className={(themeLight ? 'w3-light-grey' : 'w3-black') + " w3-round w3-padding w3-nowrap w3-overflow"}>
            ...
            glitchProduct = (
                <div style={{ padding: 8 }}>
                    <div className="w3-border w3-round w3-flex w3-flex-center-v" style={{ height: 56 }}>
                        <div style={{ paddingInline: 16 }}>
    ...
    useEffect(() => {
        ...
        if (xcode && xcode != 'null') {
            ...
            axios
                .get(`${source}/_store/all?where=prospection`)
                .then((res) => {
                    document.getElementById('productCore').style.display = 'block';
            ...
            <div>
                ...
                <div style={{ padding: 8 }}>
                    <label className="w3-margin-right">
                        ...
                        } defaultChecked={true} type="radio" name="option" value="premium" style={{ marginRight: 8 }} />
                        Premium
                    </label>
                    <label>
                        <input id="freeRadio" onChange={(e) => productInfos.type = e.target.value
                        } type="radio" name="option" value="free" style={{ marginRight: 8 }} />
                        Free
                    </label>
                </div>
